// This is code to create a singly linked list and perform operations on it.
...
{
    int data;
    struct Node *next; // pointing to an another node
...
void deleteFirstNode()
{
    struct Node *p;
    p=start;
    start=p->next;   // confused about start->next should be equal to NULL as per create() function. start stores the address of newnode, and newnode->next does exist
                    // so, start->next must be having some memory address stored in it.
    p=start;
}
